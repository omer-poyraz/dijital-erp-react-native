...
import { DataTable, Text, Card, Button, TextInput, Portal, Modal, Avatar, Switch, Divider } from 'react-native-paper';
...
import { fetchCMMQualityDescription } from '../../redux/slices/cmmQualityDescriptionSlice';
import { fetchCMMAddFile } from '../../redux/slices/cmmAddFileSlice';
import { fetchCMMAddResultFile } from '../../redux/slices/cmmAddResultFileSlice';
...
const ProjectTable = () => {
    ...
    const [resultFileModalVisible, setResultFileModalVisible] = useState(false);
    const [isResultUploading, setIsResultUploading] = useState(false);
    const cmmGetAll = useSelector(state => state.cmmGetAll.data);
    const [formData, setFormData] = useState({ note: "", description: "", status: true, partCode: "" });
    const { t } = useTranslation()
    const [isUploading, setIsUploading] = useState(false);
    const dispatch = useDispatch();
    const uploadStatus = useSelector(state => state.cmmAddFile.status);
    ...
    const getData = () => {
        setFilteredData(cmmGetAll);
    }

    useEffect(() => { getData() }, [])

    useEffect(() => {
        if (cmmGetAll && cmmGetAll.length > 0 && !selectedProject) {
            setSelectedProject(cmmGetAll[0]);
            setShowSuccessTable(cmmGetAll[0].basariliDurumlar.length > 0);
            setShowFailureTable(cmmGetAll[0].basarisizDurumlar.length > 0);
        }
    }, [cmmGetAll]);
    ...
    const handleRowPress = (item) => {
        if (selectedProject?.id === item.id) {
            navigation.navigate('CMMDetail', { id: item.id });
        }
    ...
    const handleSuccessRowPress = (item) => {
        navigation.navigate('CMMSuccessDetail', { id: item.id });
    };

    const handleFailureRowPress = (item) => {
        navigation.navigate('CMMFailureDetail', { id: item.id });
    };
    ...
    const handleFileUpload = async () => {
        ...
        try {
            ...
            await dispatch(fetchCMMAddFile({ formData, id: selectedRow.id }));
    ...
    const handleResultFileUpload = async () => {
        ...
        try {
            ...
            await dispatch(fetchCMMAddResultFile({ formData, id: selectedRow.id }));
            await dispatch(fetchCMMGetAll());

            alert(`${files.length} sonuç dosyası başarıyla yüklendi!`);
            setIsResultUploading(false);
            setResultFileModalVisible(false);
        } catch (error) {
            console.error("Sonuç dosyası yükleme hatası:", error);
            alert(`Sonuç dosyası yükleme sırasında bir hata oluştu: ${error.message}`);
            setIsResultUploading(false);
            setResultFileModalVisible(false);
        }
    };
    ...
    const handleSaveNote = async () => {
        await dispatch(fetchCMMNoteCreate({ formData: formData, manualId: selectedProject.id }));
        var data = await dispatch(fetchCMMGet({ id: selectedProject.id }));
        if (data.payload) {
    ...
    const getQualityList = async () => {
        await dispatch(fetchCMMQualityGetAllByFailure({ id }));
    }
    ...
    const to1 = Math.min((page1 + 1) * itemsPerPage, cmmGetAll?.length);
        ...
        <View style={styles.page}>
            <ScrollView>
                        ...
                        <ScrollView horizontal>
                            <DataTable style={styles.table}>
                                <DataTable.Header style={styles.tableHeader}>
                                    ...
                                    <DataTable.Title textStyle={styles.headerText} style={styles.columnHeader5}>CNC Tezgah</DataTable.Title>
                                    <DataTable.Title textStyle={styles.headerText} style={styles.columnHeader6}>Ölçüm</DataTable.Title>
                                    ...
                                    <DataTable.Title textStyle={styles.headerText} style={styles.columnHeader11}>Sonuç Raporu</DataTable.Title>
                                    <DataTable.Title textStyle={styles.headerText} style={styles.columnHeader12}>Açıklama</DataTable.Title>
                                    <DataTable.Title textStyle={styles.headerText} style={styles.columnHeader13}>Katı Model</DataTable.Title>
                                </DataTable.Header>
                                ...
                                {filteredData?.slice(from1, to1).map((item) => {
                                    ...
                                    return (
                                        <DataTable.Row key={item.id} style={[styles.tableRow, selectedProject?.id === item?.id && styles.selectedRow]} onPress={() => handleRowPress(item)}>
                                            ...
                                            <DataTable.Cell style={styles.tableCell5}><Text style={styles.cellText}>{item.stand}</Text></DataTable.Cell>
                                            <DataTable.Cell style={styles.tableCell6}><Text style={styles.cellText}>{item?.measuringPerson?.firstName} {item?.measuringPerson?.lastName}</Text></DataTable.Cell>
                                            ...
                                            <DataTable.Cell style={styles.tableCell10}>
                                                <Button
                                                    ...
                                                    disabled={isUploading || isResultUploading}
                                                >
                                                    Yükle
                                                </Button>
                                            </DataTable.Cell>
                                            <DataTable.Cell style={styles.tableCell11}>
                                                <Button
                                                    mode="outlined"
                                                    onPress={() => {
                                                        setSelectedRow(item);
                                                        setResultFileModalVisible(true);
                                                    }}
                                                    icon="file-upload"
                                                    style={styles.tableButton}
                                                    labelStyle={styles.tableButtonLabel}
                                            ...
                                            <DataTable.Cell style={styles.tableCell12}><Text style={styles.cellText}>{item.description}</Text></DataTable.Cell>
                                            <DataTable.Cell style={styles.tableCell13}><Text style={styles.cellText}>{item.solidModel}</Text></DataTable.Cell>
                                        </DataTable.Row>
                                    );
                                })}
                                ...
                                <DataTable style={styles.table}>
                                    ...
                                    {selectedProject?.basariliDurumlar.slice(from2, to2).map((item) => (
                                        <DataTable.Row key={item.id} style={styles.table2Row} onPress={() => handleSuccessRowPress(item)}>
                                            <DataTable.Cell style={styles.table2Cell1}><Text style={styles.cellText}>{item.technician ? item.technician?.firstName : item.user?.firstName} {item.technician ? item.technician?.lastName : item.user?.lastName}</Text></DataTable.Cell>
                ...
                <Portal>
                    ...
                    <Modal visible={resultFileModalVisible} onDismiss={() => !isResultUploading && setResultFileModalVisible(false)} contentContainerStyle={styles.modal}>
                        <Text style={styles.modalTitle}>Dosya Yükleme</Text>
                        <Text style={styles.modalText}>Proje: {selectedRow?.projectName}</Text>
                        <Text style={styles.modalText}>Parça Kodu: {selectedRow?.partCode}</Text>
                        ...
                        {isUploading ? (
                            ...
                            <View style={styles.modalButtons}>
                                ...
                                <Button
                                    mode="contained"
                                    onPress={handleResultFileUpload}
                                    style={styles.modalButton}
                                    buttonColor={colors.primary}
                                    loading={isResultUploading}
                                    disabled={isResultUploading}
                                >
                                    Sonuç Dosyası Seç ve Yükle
                                </Button>
                            </View>
                        )}
                    </Modal>
                ...
                >
                    ...
                    <Button
                        ...
                        onPress={async () => {
                            ...
                            await dispatch(fetchCMMQualityDescription({ formData, id: selectedFailureId }));
                            const updated = await dispatch(fetchCMMGet({ id: selectedProject.id }));
                            if (updated.payload) {
