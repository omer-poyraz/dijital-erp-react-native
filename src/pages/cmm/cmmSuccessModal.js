...
import { fetchCMMSuccessGet } from '../../redux/slices/cmmSuccessGetSlice'
import { useDispatch, useSelector } from 'react-redux'
import { useNavigation } from '@react-navigation/native'

const CMMSuccessDetailPage = ({ route }) => {
    const { id } = route.params
    const { t } = useTranslation();
    const data = useSelector(state => state.cmmSuccessGet.data);
    ...
    const getData = async () => {
        await dispatch(fetchCMMSuccessGet({ id: id }))
    }
...
export default CMMSuccessDetailPage;