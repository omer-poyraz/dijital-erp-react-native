...
import CMMNoteModal from './cmmNoteModal'
import PdfRendererView from 'react-native-pdf-renderer'
import ViewShot from 'react-native-view-shot'

const CMMDetailPage = ({ route }) => {
    const { id } = route.params;
    const { t } = useTranslation();
    const [activeTab, setActiveTab] = useState('details');
    const [selectedImage, setSelectedImage] = useState(null);
    const [noteModal, setNoteModal] = useState(false);
    const data = useSelector(state => state.cmmGet.data);
                        ...
                        <Card.Content>
                            ...
                            <View style={styles.statsRow}>
                                ...
                                <View style={styles.statItem}>
                                    ...
                                    <Text style={styles.statCount}>{data?.cmmNotes?.length || 0}</Text>
    ...
    }, [navigation, id, data, selectedImage])
    ...
    return (
        <View style={styles.page}>
            ...
            <CMMNoteModal modal={noteModal} item={data} setModal={setNoteModal} id={id} />
        </View>
    );
};
